 ...
 * 2 bzw. 4 nebeneinander. Linie oder Kerzen, Tageskurse oder Intraday. Ein
 * Klick in der Watchlist setzt den aktiven Slot; ein leerer Slot bietet die
 * Watchlist zum Zuweisen.
 */
...
interface Cell { loading?: boolean; closes?: number[]; ohlc?: Ohlc[]; t?: number[]; intraday?: boolean; error?: string; demo?: boolean; }
...
export default function ChartView({ focus }: { focus: string | null }) {
  ...
  const loadDemo = (sym: string) => {
    ...
    setCache((c) => ({ ...c, [keyOf(sym)]: { demo: true, closes: cl, ohlc: toOhlc(cl), t: demoTimes(cl.length, def.stepSec), intraday: def.intraday } }));
  };
            ...
            <div key={i} className={`${s.slot} ${isActive ? s.slotOn : ""}`} onClick={() => setActive(i)}>
              {sym ? (
                <>
                  ...
                  {series ? (
                    <>
                      <div className={s.slotChart}><BigChart data={series} candles={cell?.ohlc} times={cell?.t} intraday={cell?.intraday} mode={mode} /></div>
